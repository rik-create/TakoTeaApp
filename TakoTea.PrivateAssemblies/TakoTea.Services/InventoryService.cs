...
using TakoTea.Models;  // Ensure this namespace is correct
namespace TakoTea.Services
{
    public class InventoryService : IInventoryService
    {
        ...
        {
            using (SqlConnection connection = _dbConn)
            {
                connection.Open();
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        int ingredientID = connection.QuerySingle<int>(
                        ...
                        foreach (int linkedProductId in linkedProductIds)
                        {
                            _ = connection.Execute(
